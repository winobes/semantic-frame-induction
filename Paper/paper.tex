...
Verbargs is the subset of Google's syntactic-ngram corpus for training our 
models and evaluating the reults 
A syntactic-ngram is a $k$-word rooted subtree of some sentence.
The ngrams in Google's corpus were extracted from 3.5 million books and are labled with
Penn Treebank part-of-speech tags Stanford-basic dependency lables \citep{ngrams2013}.
Each ngram also includes frequency counts by year.
In this project, used the 

The ngrams in the verbargs dataset (130M items) consist of a verb and all of its 
immediate arguments.
From this dataset, we consider only VSO's, that is; we pruned verbargs to only
...
we could not used document-level context to leverage semantic information.
...
or object.
More advanced models would take better advantage of the various kinds of arguments
in verbargs and their relation to one another.
For example, you might be gleen some semantic information about the relationship between
verbs and prepositional objects by the preposition that mediates them: 
That you ''swim \emph{in} the lake'' versus ``ice skate \emph{on} the lake'' 
